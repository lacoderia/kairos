...
CSV.open("payments_omein_#{ARGV[0]}_#{ARGV[1]}.csv", "wb") do |csv|
  csv << ["ID OMEIN", "NOMBRE", "APELLIDO", "EMAIL", "RANGO", "$ NIVEL 1", "NIVEL 1 IDS", "$ NIVEL 2", "NIVEL 2 IDS", "$ NIVEL 3", "NIVEL 3 IDS", "$ NIVEL 4", "NIVEL 4 IDS", "$ NIVEL 5", "NIVEL 5 IDS", "$ NIVEL 6", "NIVEL 6 IDS", "$ NIVEL 7", "NIVEL 7 IDS", "$ NIVEL 8", "NIVEL 8 IDS", "$ NIVEL 9", "NIVEL 9 IDS", "$ TOTAL" ]
  ...
  users.uniq.each do |user|
    ...
    summary = user.summaries.where(period_start: PERIOD_START, period_end: PERIOD_END).first
