feature 'ShippingAddressesController' do
  ...
  context 'deactivate shipping addresses for user' do

    let!(:user_01){create(:user, :confirmed, :with_address)}
    let!(:user_02){create(:user, :confirmed, :with_address)}

    it 'should deactivate shipping address for logged in user' do
      
      with_rack_test_driver do
        expect {page.driver.post deactivate_shipping_address_path(user_01.shipping_addresses.first.id), {} }.to raise_error.with_message('You are not authorized to access this page.')
      end

      expect(user_01.shipping_addresses.available.count).to eql 1
      ...
      with_rack_test_driver do
        page.driver.post deactivate_shipping_address_path(user_01.shipping_addresses.first.id), {} 
      end
      
      response = JSON.parse(page.body)

      expect(page.status_code).to be 200
      expect(user_01.shipping_addresses.available.count).to eql 0
